...
public interface RetrofitService {
    ...
    //4. GET방식으로 값을 보낼때 Map collection 을 이용하여 한방에 값 전달하기
    @GET("Retrofit/getTest.php")
    Call<Item> getMethodTest2(@QueryMap Map<String,String> datas);

    //5. POST방식으로 값 전달 [ @Body ] - 객체를 전달하면 자동으로 객체멤버변수를 json문자열로 변환하여 서버로 전송해 줌.
    ...
    //7. GET방식으로 json array 값 읽어와서 ArrayList<Item> 로 곧바로 파싱
    @GET("Retrofit/boardArray.json")
    Call<ArrayList<Item>> getBoardArray();

    //8. GSON을 통해 자동으로 Item객체로 파싱하지 않고 그냥 문자열로 응답결과 받아보기
    @GET("Retrofit/board.json")
