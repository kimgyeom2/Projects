...
import com.mrhi2023.ex82httprequest.databinding.ActivityBoardBinding;
...
//        adapter.notifyDataSetChanged();
        ...
        new Thread(){
            @Override
            public void run() {
                //서버 DB값을 echo해주는 php 문서를 실행
                String serverAddress="http://mrhi2023.dothome.co.kr/Android/loadDB.php";
...
//                    });
                    ...
                    //서버에서 echo된 문자열데이터에서 '&'문자를 기준으로 문자열을 분리
                    //한줄 단위(Item)로 데이터를 분리..
                    String[] rows= buffer.toString().split("&");
                    Log.i("TAG", rows.length +"");

                    //한줄 데이터의 콤마구분자를 분리하여 값들 분석하기[csv 파싱]
                    for(String row: rows){
